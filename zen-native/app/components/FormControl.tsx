...
import {InputModeOptions, StyleSheet, Text, TextInputProps, View} from 'react-native';
import {applyCpfMask, applyPhoneMask} from "@/app/utils/maskUtils";
...
const FormControl: React.FC<FormControlProps> = ({
    ...
    const handleTextChange = (text: string) => {
        let maskedText = text;
        if (type === 'phone') {
            maskedText = applyPhoneMask(text);
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.inputContainer, error ? styles.inputError : null]}>
                {icon && <FontAwesome name={icon} size={16} style={styles.icon}/>}
                <Input
            ...
            {helperText && <Text style={[styles.helperText, error ? styles.helperTextError : null]}>{helperText}</Text>}
        </View>
    );
};
